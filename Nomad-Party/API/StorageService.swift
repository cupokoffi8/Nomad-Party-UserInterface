...
//                }
    ...
    static func savePhotoProfile(image: UIImage, uid: String, onSuccess: @escaping() -> Void, onError: @escaping(_ errorMessage: String) -> Void) {
        ...
        let storageProfileReference = Ref().storageSpecificProfile(uid: Api.User.currentUserId)
        
        let metadata = StorageMetadata()
        metadata.contentType = "image/jpg"
        
        storageProfileReference.putData(imageData, metadata: metadata, completion: { (storageMetaData, error) in
            if error != nil {
               onError(error!.localizedDescription)
                return
            }
    ...
    static func savePhoto(username: String, uid: String, data: Data, metadata: StorageMetadata, storageProfileReference: StorageReference, dict: Dictionary<String, Any>, onSuccess: @escaping() -> Void, onError: @escaping(_ errorMessage: String) -> Void) {
        storageProfileReference.putData(data, metadata: metadata, completion: { (storageMetaData, error) in
             if error != nil {
                onError(error!.localizedDescription)
                 return
             }
             ...
             storageProfileReference.downloadURL(completion: { (url, error) in
                 if let metaImageUrl = url?.absoluteString {
                    ...
                    if let changeRequest =
                        // Current user data is stored in database
